Спецификация программы ChatList:
1. Назначение программы

ChatList — это Python-приложение, которое позволяет отправлять один и тот же промпт в несколько нейросетей и сравнивать их ответы. Пользователь вводит текст запроса, программа отправляет его в выбранные модели и показывает результаты в виде временной таблицы. Отмеченные пользователем строки сохраняются в постоянную БД.

2. Стек

- Python 3.11+
- SQLite 
- Любая библиотека для HTTP-запросов (requests / httpx)
- GUI PyQt 

3. Таблицы базы данных
Программа хранит следующие данные в базе данных SQLite:
- Промпты (prompts) - таблица запросов. Поля: дата, промпт, теги
- Сети (models) - таблица нейросетей. Поля: name, api-url, api-id, is_active
То есть хранить в таблице на API-ключи, а имя переменной, где ключ лежит. Сами API-ключи будут в файле .env
- Результаты (results) - таблица сохраненных результатов.
- Настройки (settings) - хранение настроек программы


4. Рабочий процесс
4.1 Ввод промпта

Пользователь:

- вводит новый промпт, или
- выбирает один из сохранённых в таблице prompts.

4.2 Отправка запроса

Программа:

- Берёт список активных моделей (models.where(is_active=1)).
- Отправляет промпт в каждую модель.
- Создаёт временную таблицу результатов (в памяти, не в SQLite):

4.3 Интерфейс результатов

На экране пользователь видит таблицу, где каждая строка содержит:

- название модели
- текст ответа
- чекбокс selected

4.4 Сохранение выбранных результатов

Когда пользователь нажимает Сохранить:

- Все строки с selected = True записываются в таблицу results.
- Временная таблица очищается.

4.5 Новый запрос

При вводе нового промпта временная таблица:

- удаляется полностью,
- создаётся заново после получения новых ответов.

5. Требования к реализации

- Доступ к SQLite должен быть инкапсулирован в модуле db.py.
- Логика работы с моделями — в модуле models.py.
- Отправка запросов к API — модуль network.py.
- Основной интерфейс — main.py.

6. Дополнительные функции (опционально)

- Во всех таблицах должен быть реализован поиск и сортировка.
- Экспорт выбранных результатов в Markdown / JSON.
- Поддержка разных типов моделей (OpenAI, DeepSeek, Groq, Claude, Gemma, Llama, Qwen и т.д.).
- Логи запросов.

7. Функция "AI-ассистент для улучшения промптов", которая:

1. Принимает исходный текст промпта от пользователя.
2. Отправляет его в одну указанную модель (через существующий OpenRouter-клиент).
3. Возвращает:
   - улучшенную версию промпта;
   - 2–3 варианта переформулировки;
   - при необходимости — адаптацию промпта под разные типы моделей (код, анализ, креатив).
4. Встраивает это в UI:
   - отдельная кнопка «Улучшить промпт» рядом с полем ввода;
   - окно/панель, где пользователь видит:
     - исходный промпт;
     - улучшенный;
     - альтернативные варианты;
   - возможность «Подставить в поле ввода» выбранный вариант одним кликом.

8. Функция "Настройки":
   1. Позволяет выбрать светлую/темную тему
   2. Позволяет выбрать размер шрифта панелей
   3. Все настройки сохраняются в таблицу settings в базе данных chatlist.db

9. Функция "О программе", которая выводит краткую информацию о программе.   
